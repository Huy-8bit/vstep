...
from app.api.routes import auth, progress, writing
...
app = FastAPI(title="VSTEP Writing Lab", version="1.0.0", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
...
async def request_guards(request: Request, call_next):
    if request.method in {"POST", "PATCH", "PUT", "DELETE"}:
        ...
        if path.endswith(("/login", "/register", "/grade", "/generate")):
            now = time.monotonic()
...
for router in (auth.router, writing.router, progress.router):
