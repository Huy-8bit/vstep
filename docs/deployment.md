...
Đăng nhập qua `/login` thông thường và kiểm tra `/admin`. Sau khi tạo thành công: thay mật khẩu nếu muốn, xóa `INITIAL_ADMIN_PASSWORD` khỏi môi trường và đặt `INITIAL_ADMIN_ENABLED=false`, rồi khởi động lại. Tài khoản Admin vẫn tồn tại. Có thể dùng CLI: `python -m app.cli.create_admin --email admin@example.com --name Administrator --new-account`; CLI hỏi mật khẩu tương tác. Nếu Admin cũ không đăng nhập được, dùng [`check_admin` và `reset_admin_password`](admin-auth-troubleshooting.md) để khôi phục tường minh. Với tài khoản có sẵn, bỏ `--new-account` để nâng quyền mà không đổi mật khẩu.
