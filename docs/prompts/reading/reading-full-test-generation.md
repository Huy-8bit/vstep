# Sinh Full Reading 4 passages / 40 câu
...
KẾT QUẢ DỄ ĐỌC:
Đề hoàn chỉnh; key riêng; explanation cho từng option và evidence; metadata internal bands riêng không lộ trong đề. JSON ManualReadingFullTest chứa 4 GeneratedReadingPassage; wrapper thủ công, không phải endpoint tạo full test.
```
...
Schema đối chiếu: [ManualReadingFullTest](../reference/schemas/ManualReadingFullTest.schema.json).
