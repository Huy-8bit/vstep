# Speaking all-in-one — chỉ văn bản

Đồng bộ: 2026-09-19 · source commit `658986a` · manual kit `1.0.0`.

Phiên bản nguồn: SPEAKING_GRADER_PROMPT_VERSION=2.1.0; SPEAKING_CORRECTION_PROMPT_VERSION=1.0.0; ESCALATION_VERSION=1.5.0; VOCABULARY_COACH_VERSION=1.0.0.

## Purpose

Chấm, sửa, gợi ý câu trả lời và học từ từ transcript.

## When to use

Dán một hoặc nhiều câu hỏi và transcript; không cần audio.

## Required input

- `{{QUESTION}}`: Đề hoặc danh sách câu hỏi đánh số.
- `{{TRANSCRIPT}}`: Nguyên văn từng câu trả lời, khớp đề.

## Optional input

- `{{PART}}`: 1/2/3; 0 nếu đủ full test; không rõ thì chỉ nhận xét ngôn ngữ, không suy diễn yêu cầu part.
- `{{PREVIOUS_WEAKNESSES}}`: Chỉ lịch sử thật.
...
{{QUESTION}}
==========================================
Đề hoặc danh sách câu hỏi đánh số.
================ TRANSCRIPT ================
{{TRANSCRIPT}}
==========================================
Nguyên văn từng câu trả lời, khớp đề.
================ PART ================
{{PART}}
==========================================
1/2/3; 0 nếu đủ full test; không rõ thì chỉ nhận xét ngôn ngữ, không suy diễn yêu cầu part.
================ PREVIOUS_WEAKNESSES ================
{{PREVIOUS_WEAKNESSES}}
==========================================
Chỉ lịch sử thật.
...
VOCABULARY_COACH=yes; ưu tiên spoken chunks tự nhiên thay vì cụm văn viết trang trọng. Thêm bài học ngắn về cách dùng từ, ví dụ từ ý người học và một câu hỏi active recall. Không cho điểm Content riêng.
...
Bảng 5–10 cụm nói hữu ích và bài học từ vựng. JSON giữ vocabulary_suggestions theo SpeakingTextGradingOutput; bài học mở rộng chỉ ở chế độ human.
```
...
Human kết hợp Vocabulary Coach/bài học cho tiện. JSON không thêm các trường ngoài text schema. Không nhập lại grading vào app.
